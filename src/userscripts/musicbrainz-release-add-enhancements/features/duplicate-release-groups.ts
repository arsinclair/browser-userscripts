...
function findDuplicateRow(): HTMLTableRowElement | undefined {
    return (
        document.querySelector(DUPLICATE_LIST_SELECTOR)?.closest<HTMLTableRowElement>("tr") ??
        undefined
    );
}
...
export function initDuplicateReleaseGroupsToggle(): void {
    ...
    const update = (): void => {
        if (!controlRow?.isConnected) {
            const releaseGroupRow = document
                .querySelector<HTMLInputElement>("#release-group")
                ?.closest("tr");
            if (!releaseGroupRow) {
                return;
            }
