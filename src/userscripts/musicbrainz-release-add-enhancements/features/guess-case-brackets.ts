...
const GUESS_CASE_BUTTON_SELECTOR =
    'button.guesscase-title, #guess-case-button, button[data-bind*="guessMediumCase"]';
...
function affectedTitleInputs(button: HTMLButtonElement): HTMLInputElement[] {
    if (button.id === "guess-case-button") {
        return Array.from(
            document.querySelectorAll<HTMLInputElement>(`#tracklist ${TITLE_INPUT_SELECTOR}`)
        );
    }
    ...
    if (button.matches('[data-bind*="guessMediumCase"]')) {
        return Array.from(
            button
                .closest(".advanced-medium")
                ?.querySelectorAll<HTMLInputElement>(TITLE_INPUT_SELECTOR) ?? []
        );
    }
