    ...
    return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            onerror: response =>
                reject(
                    new Error(
                        `The MusicBrainz request failed (HTTP ${response.status || "unknown"}).`
                    )
                ),
...
async function fetchRelease(releaseId: string): Promise<MusicBrainzReleaseResponse> {
    ...
    for (let attempt = 0; ; attempt += 1) {
        try {
            ...
            console.info(
                `[Navidrome MusicBrainz Links] MusicBrainz is busy; retrying in ${retryDelay / 1_000}s.`
            );
...
function scan(): void {
    for (const link of document.querySelectorAll<HTMLAnchorElement>(
        'a[href*="musicbrainz.org/release/"]'
    )) {
