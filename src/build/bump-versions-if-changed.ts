...
async function main(): Promise<void> {
    try {
        execSync(`git fetch --depth=1 origin ${DEPLOY_BRANCH}`, { stdio: "inherit" });
    } catch {
        console.log(`No remote ${DEPLOY_BRANCH} branch found; skipping version bumps.`);
        return;
    }
    ...
    if (githubOutput) {
        const lines = [
            `changed=${bumped.length > 0 ? "true" : "false"}`,
            `bumped=${JSON.stringify(bumped)}`
        ];
